<!--
  AIAssistant.vue — 掼蛋智能助手组件

  功能说明：
    提供悬浮球 + 聊天窗口形式的AI助手入口，支持：
    1. 拖拽悬浮球：用户可随意拖动悬浮球到页面任意位置
    2. AI聊天：输入问题向后端 /api/ai/chat 发送请求，获取回答
    3. 规则问答入口：用户可输入掼蛋规则相关问题获取说明
    4. 未读消息红点：新消息到达时显示红色角标
  ...
  入口暴露方式：
    该组件暴露为全局 AI 助手入口，通过 float-ball 悬浮在页面右上角。
    父组件引入后直接放置在 App.vue 或 Layout 组件中即可全局生效。

  回归验证点：
    [TC-AI-UI-001] 悬浮球默认显示在右下角，可拖拽移动
    [TC-AI-UI-002] 点击悬浮球打开聊天窗口，再次点击关闭
    [TC-AI-UI-003] 聊天窗口打开时拖拽悬浮球无效（不触发拖拽）
    [TC-AI-UI-004] 拖拽范围限制在窗口可视区域内
    [TC-AI-UI-005] 初次加载显示欢迎消息
    [TC-AI-API-001] 输入空消息点击发送 → 无请求发起
    [TC-AI-API-002] 输入超长消息(>500字) → ElMessage.warning 提示，不发起请求
    [TC-AI-API-003] 输入正常消息 → 调用 /api/ai/chat，显示用户消息和加载动画
    [TC-AI-API-004] AI 返回空响应 → 兜底显示"抱歉，我暂时无法回答。请稍后再试。"
    [TC-AI-API-005] AI 请求网络异常 → ElMessage.error 显示"AI助手暂时无法连接"
    [TC-AI-API-006] 连续快速点击发送 → isSubmitting 防止重复提交
-->
