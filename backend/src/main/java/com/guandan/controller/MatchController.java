 ...
 *
 * ── 匹配模块配置说明 ──────────────────────────────────
 * 本控制器与 MatchService、ScheduleConfig 组成匹配模块：
 * - MatchController : API 入口，负责请求/响应
 * - MatchService : 匹配业务逻辑，队列维护和匹配执行
 * - ScheduleConfig : 定时触发匹配检测（每 3 秒）
 *
 * ── 取消匹配配置说明 ──────────────────────────────────
 * - 取消时同步清理队列和结果缓存（roomCache.removeMatchResult）
 * - 不在队列中取消返回 400 错误
 * - 幂等设计：已在队列时重复加入直接返回成功
 *
 * ── 结果轮询配置说明 ──────────────────────────────────
 * - 建议轮询间隔：2-3 秒
 * - 匹配结果通过 roomCache.getMatchResult 获取
 * - 轮询超时 60 秒，前端负责处理超时逻辑
 * - 匹配成功后前端消费结果并清理缓存
 * ─────────────────────────────────────────────────────
 */
@Slf4j
